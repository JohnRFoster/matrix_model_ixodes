...
model.code <- nimbleCode({
  ...
  for(t in 1:N_days){   # loop over every day in time series
    ...
    logit(l2n[t]) <- theta.ln + beta.m[1]*mice[t]
    logit(n2a[t]) <- theta.na + beta.m[2]*mice[t]
  ...
  for(t in 1:(N_est-1)){
    ...
    Ex[1:ns,t] <- P[1:ns,1:ns,start[t]] %*% x[1:ns,t] 
